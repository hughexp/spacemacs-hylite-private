...
(defun hylite/directory-parent (directory)
...
;; Screenshot
(defun hylite//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun hylite/capture-screenshot (basename)
  ...
  (if (file-exists-p (file-name-directory fullpath))
      (progn
        ...
        (hylite//insert-org-or-md-img-link "../img/" relativepath))
    (progn
      (call-process "screencapture" nil nil nil "-s" (concat basename ".png"))
      (hylite//insert-org-or-md-img-link "./" (concat basename ".png"))))
  (insert "\n"))

(defun hylie/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun hylite/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun hylite/skip-non-stuck-projects ()
...
(defun hylite/org-insert-src-block (src-code-type)
...
(defun hylite/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-log-states)    ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(defun hylite/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun hylite/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
    ...
    (while (setq file (pop files))
      ...
      (with-current-buffer org-agenda-buffer
        (dolist (current-tag include-tags)
          (org-clock-sum tstart tend 'hylite/filter-by-tags)
